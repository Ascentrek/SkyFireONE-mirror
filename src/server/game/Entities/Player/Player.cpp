...
void Player::SendLoot(uint64 guid, LootType loot_type)
{
    ...
    else if (IS_ITEM_GUID(guid))
    {
        ...
        else
        {
            ...
            if (!item->m_lootGenerated)
            {
                item->m_lootGenerated = true;
                loot->clear();
                loot->FillLoot(item->GetEntry(), LootTemplates_Item, this, this);
    ...
    else if (IS_CORPSE_GUID(guid))                          // remove insignia
    {
        ...
        if (!bones->lootForBody)
        {
            ...
            if (Battleground *bg = GetBattleground())
                if (bg->GetTypeID() == BATTLEGROUND_AV)
                    loot->FillLoot(1, LootTemplates_Creature, this, this);
        ...
        if (loot_type == LOOT_PICKPOCKETING)
        {
            if (!creature->lootForPickPocketed)
            {
                ...
                if (uint32 lootid = creature->GetCreatureTemplate()->pickpocketLootId)
                    loot->FillLoot(lootid, LootTemplates_Pickpocketing, this, this);
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    SetUInt32Value(PLAYER_BYTES_3, (fields[49].GetUInt16() & 0xFFFE) | fields[5].GetUInt8()); //check player_bytes_3
    ...
    std::string taxi_nodes = fields[37].GetCppString();  // check this 37=dungeon_difficulty
